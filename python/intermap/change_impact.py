...
def _scan_project_files(project_path: str, language: str = "python") -> list[str]:
    """Scan project for source files using workspace iteration."""
    extensions = _EXT_MAP.get(language, {".py"})
    return [str(p) for p in iter_workspace_files(Path(project_path), extensions=extensions)]


def _get_imports_from_file(file_path: str) -> list[dict]:
    """Extract imports from a file using the extractor."""
    try:
        result = _extractor.extract(file_path)
        imports = []
...
def find_tests_importing_module(
    ...
    try:
        all_files = _scan_project_files(str(project), language=language)
